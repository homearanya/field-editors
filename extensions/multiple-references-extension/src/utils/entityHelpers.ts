...
export function getEntityDescription({
  ...
  const isTextField = (field: ContentTypeField) => ['Symbol', 'Text', 'Date'].includes(field.type);
  ...
  const descriptionField2 = contentType.fields
    .filter((field) => field.id !== descriptionField.id)
    .find(isDescriptionField);

  return descriptionField2
    ? `${getFieldValue({
        entity,
